    ...
    IconObj = LibStub("LibDataBroker-1.1"):NewDataObject(addonName, {
      ...
      OnClick = function(self, button)
        ...
        elseif button == "MiddleButton" then
          ...
          addon:Print(L["If you want to show the minimap icon again, enable it in the \"Enabled Modules\" section of the addon."])
        end
      end,
      OnTooltipShow = function(tooltip)
        tooltip:AddLine(L[addonName])
        tooltip:AddLine(" ")
        tooltip:AddLine(L["Left click: Open settings."])
        tooltip:AddLine(L["Middle click: Hide minmap icon."])
      end,
    })
